<!--
  pages/standard-answers/[id].vue — 표준답변 상세·편집 전용 페이지 (003 연동).
  /standard-answers/new = 신규(POST), /standard-answers/:id = 기존(GET 로드 → PATCH 저장).
  섹션: 라벨 · 질문(AdminRichEditor) · 답변(AdminRichEditor) · 분류(scope/서비스/토픽/태그) ·
        중복 경고(생성 응답 similar[]) · 버전 체인 배너 · 최신성 배지 ·
  ...
  API:
    ...
    POST   /standard-answers   body {label,question,answer,scope?,topicId?,serviceId?,tags?}  응답 {id, similar[]}
    PATCH  /standard-answers/:id            본문 수정 (admin)
    PATCH  /standard-answers/:id/transition body {to, reason?, archivedReason?}  전이 (developer↑)
    DELETE /standard-answers/:id            soft delete (admin)
    GET    /topics · /services             분류 카탈로그 (셀렉트 옵션)
  QUALITY §10-4 승인 UI:
    - CK 체크리스트(자동 2 + 수동 8) — reviewing 상태 전용 패널
...
import { ArrowLeft, Trash2, Save, AlertTriangle, CheckCircle2, XCircle, MinusCircle, GitBranch, Clock } from 'lucide-vue-next'
...
/* PII 게이트 컬럼 (P2-EXPOSURE-GUARD §4-B) — 상세 응답(sa.*)에 포함됨 */
...
const current = ref<SARow | null>(null)   // 편집 대상 원본(상태 배지·전이용). 신규는 null.
...
async function fetchOne(id: number | string): Promise<SARow> {
  ...
  return (await res.json()) as SARow
}
...
useHead(() => ({
  title: `${isNew.value ? '새 표준답변' : (form.label || '표준답변 편집')} · 맑은도우미 Admin`,
}))
...
async function refreshVerificationBadge() {
  try {
    ...
    badges.value = {
      ...badges.value,
      'standard-answers-verify': d.total != null && d.total > 0
...
async function doSave() {
  ...
  try {
    if (!isNew.value && current.value) {
      ...
      // 중복 후보가 있으면 페이지를 떠나지 않고 경고 — 운영자 확인
...
const manualCks = reactive<Record<string, boolean>>({
  ck5: false,   // 분류·라벨 정합 (CK-5)
  ck6: false,   // 정확성 B1~B5 (CK-6)
  ck7: false,   // 명확성 C1~C5 (CK-7)
  ck8: false,   // 톤·격식 E1~E5 (CK-8)
  ck9: false,   // 구조·마크업 D1~D4 (CK-9)
  ck10: false,  // 최신성 (CK-10)
  ck11: false,  // PII·비공개 없음 (CK-11)
  ck12: false,  // 중복 없음 (CK-14 재사용성 포함)
})

const MANUAL_CK_META: { key: keyof typeof manualCks; label: string; hint: string }[] = [
  { key: 'ck5',  label: '분류·라벨 정합',       hint: '지정된 topic이 본문 주제와 실제 일치함 확인 (CK-5)' },
  { key: 'ck6',  label: '정확성',                hint: 'B1~B5 — 현행 제품·정책과 일치, 추측 없음 (CK-6)' },
  { key: 'ck7',  label: '명확성·가독성',         hint: 'C1~C5 — C 평균 4점 이상, 단계 번호화·전문용어 풀어쓰기 (CK-7)' },
  { key: 'ck8',  label: '톤·격식',               hint: 'E1~E5 — E 평균 3점 이상, 분류·상황 맞는 톤 (CK-8)' },
  { key: 'ck9',  label: '구조·마크업',           hint: 'D1~D4 — 챗봇 렌더링 안전, 이미지 절대 URL (CK-9)' },
  { key: 'ck10', label: '최신성',                hint: 'last_verified_at 180일 이내 또는 이번 승인이 최신성 확인 포함 (CK-10)' },
  { key: 'ck11', label: 'PII·비공개 없음',       hint: '본문·이미지 캡션 PII 없음, 주민번호 등 고유식별정보 없음 (CK-11/12/13)' },
  { key: 'ck12', label: '재사용 가능·중복 없음', hint: '특정 날짜·고객사 조건에 묶이지 않음, ERR_DUPLICATE 없음 (CK-14)' },
]

/* reviewing 상태일 때만 체크리스트 패널 표시 */
const showCkPanel = computed(() => current.value?.approval_status === 'reviewing')

/* 승인 게이트: 자동 fail 1개라도 있거나 수동 미체크 있으면 block */
...
/* reviewing → approved 직전 체크리스트 통과 재확인 (ckApproveBlocked 이중화) */
function ckGateOk(): boolean {
  return !ckApproveBlocked.value
}

/* 체크박스 상태 리셋 (페이지 이동/행 재조회 시) */
function resetCks() {
...
/* ── PII 승인 게이트 (P2-EXPOSURE-GUARD §4-2) ──
   이미지 보유 시 image_pii_status ∈ {clear,removed,masked,none} 아니면 approved 전이 차단.
   텍스트 blocked 면 approved 차단. UI 선차단 + api 422 이중화. */
...
const REJECT_CODES: { code: string; label: string; hint: string }[] = [
  ...
  { code: 'ERR_DUPLICATE',  label: '중복',             hint: '동일 topic에 이미 approved 표준답변이 존재' },
]
...
const rejectReasonValid = computed(() => {
  const hasCodes = Object.values(rejectCodeMap).some(v => v)
  return hasCodes // 코드 1개 이상 필수
})
...
async function doTransition(to: ApprovalStatus, overrideReason?: string) {
  ...
  // 보관 전이 시 archived_reason 필수 — 모달에서 이미 선택 확인 후 호출됨
  ...
  try {
    ...
    if (res.status === 422) {
      // CK 자동 게이트 거부 (ERR_CLASSIFY / ERR_PII)
...
/* ── PII 검수 후 current 재조회 (게이트 상태 반영) ── */
...
<template>
  <div>
    ...
    <div class="mb-5 flex items-center justify-between gap-3">
      ...
      >
        <ArrowLeft class="size-4" />표준답변 목록
      </NuxtLink>
      <div v-if="ready && !notFound" class="flex items-center gap-2">
        ...
        <button
          ...
          @click="doSave"
        >
          <Save class="size-4" />{{ saving ? '저장 중…' : (isNew ? '추가' : '본문 저장') }}
        </button>
      </div>
    </div>
    ...
    <template v-else-if="ready">
      ...
      <header class="mb-6">
        ...
        <div class="mt-0.5 flex flex-wrap items-center gap-2.5">
          ...
          <!-- archived_reason 표기 (QUALITY §10-4) -->
          <span
          ...
          <!-- 최신성 배지 (QUALITY §2-⑦, §10-4) — 컬럼 미배포 시 숨김 -->
        ...
        <div
          v-if="current && current.approval_status === 'rejected' && current.rejection_reason"
      ...
      <!-- 버전 체인 배너 (QUALITY §6, §10-4) — 컬럼 미배포 시 숨김 -->
      <div
        v-if="versionChain"
      ...
      >
        ...
        <div v-if="versionChain.supersedes" class="flex items-center gap-2 text-[12px] text-slate-600">
          ...
          <NuxtLink
            :to="`/standard-answers/${versionChain.supersedes}`"
            class="text-primary-600 hover:underline"
          >#{{ versionChain.supersedes }} 보기</NuxtLink>
          <span class="text-slate-400">— 이 답변이 해당 버전을 대체함</span>
        </div>
        <div v-if="versionChain.supersededBy" class="flex items-center gap-2 text-[12px] text-slate-600">
          ...
          <NuxtLink
            :to="`/standard-answers/${versionChain.supersededBy}`"
            class="text-primary-600 hover:underline"
          >#{{ versionChain.supersededBy }} 보기</NuxtLink>
          <span class="text-slate-400">— 이 답변을 대체한 최신 버전</span>
        </div>
      </div>

      <div v-if="!canWrite" class="mb-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2.5 text-[12px] text-amber-800">
        조회 전용입니다. 표준답변 본문 저장·삭제는 admin 권한이 필요합니다.
      </div>

      <!-- 중복 경고 패널 (저장 응답 similar[]) -->
      <div
        v-if="similar.length"
        class="mb-5 rounded-lg border border-amber-300 bg-amber-50 p-3"
      >
        ...
        <ul class="space-y-1.5">
          ...
          >
            ...
            <NuxtLink
              :to="`/standard-answers/${s.id}`"
              class="flex-1 truncate text-[12px] text-slate-700 hover:underline"
            >
        ...
        <p class="mt-2 text-[11px] text-amber-700">
          ...
          <NuxtLink to="/standard-answers" class="font-semibold underline">목록으로</NuxtLink>
        </p>
      </div>
      ...
      <div class="space-y-6">
        ...
        <AdminSettingsSection
          title="본문"
          description="라벨은 운영자 식별용, 질문·답변은 챗봇 노출 내용입니다. (라벨·질문·답변 필수)"
        >
          <AdminFormRow label="라벨" required>
            <input
              v-model="form.label"
              type="text"
          ...
          <AdminFormRow label="질문" required hint="대표 질문 문장">
            <AdminRichEditor
          ...
          <AdminFormRow label="답변" required hint="표준 답변 내용">
            <AdminRichEditor
              v-model="form.answer"
              :disabled="!canWrite"
              :height="720"
        ...
        >
          <AdminFormRow label="분류" required>
        ...
        <!-- 4. CK 승인 체크리스트 (reviewing 상태 전용, QUALITY §3, §10-4) -->
        <div
          v-if="current && showCkPanel"
        ...
        >
          <div class="mb-5 border-b border-amber-200 pb-4">
            ...
            <p class="mt-1 text-[12px] text-amber-700">
              QUALITY §3 — 자동 항목(A) 2개 + 수동 항목(M) 8개 전부 통과 시 승인 버튼이 활성화됩니다.
            </p>
          </div>
          <div class="space-y-4">
            <!-- 자동 항목 (읽기전용) -->
        ...
        <AdminSettingsSection
          v-if="current"
          title="승인 워크플로"
          description="검증된 답변만 챗봇에 노출됩니다. 승인/반려/보관은 developer 이상 권한이 필요합니다."
        >
        ...
        <div class="flex items-center justify-end gap-2 pb-2">
          ...
          <button
            ...
            @click="doSave"
          >
            <Save class="size-4" />{{ saving ? '저장 중…' : (isNew ? '표준답변 추가' : '본문 저장') }}
          </button>
        </div>
      </div>
    </template>
    ...
    <!-- 삭제 확인 -->
    <AdminModal v-model="confirmDelete" title="표준답변 삭제" size="sm">
      <p class="text-[13px] text-slate-700">
        <span class="font-semibold">{{ form.label }}</span> 을(를) 삭제하시겠습니까?<br />
        삭제 후 복구할 수 없습니다.
      </p>
      <template #footer>
    ...
    <AdminModal v-model="rejectModal" title="반려 사유 선택" size="lg">
      <div class="space-y-4">
        ...
        <div
          v-if="!rejectReasonValid"
          class="text-[11px] text-rose-600"
        >ERR_* 코드를 1개 이상 선택해야 반려할 수 있습니다.</div>
      </div>
      <template #footer>
        <div class="flex items-center justify-end gap-2">
          <button
