...
async function load() {
  pending.value = true
  try {
    const res = await apiFetch(`${API_BASE}/admin/kpi`, { credentials: 'include', cache: 'no-store' })
...
<template>
  <div>
    <AdminPageHeader
      caption="DASHBOARD"
      title="홈"
      description="시스템 현황과 최근 활동을 한눈에 봅니다."
    />
    ...
    <template v-else-if="kpi">
      ...
      <section class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <AdminKpiCard
          label="이번 달 비용"
          :value="fmtCost(kpi.monthCostUsd)"
          :sub="`호출 ${kpi.monthCalls.toLocaleString()}회`"
      ...
      <section class="mt-8 rounded-xl border border-slate-200 bg-white">
        ...
        <div
          v-else
          class="flex min-h-[120px] items-center justify-center text-[13px] text-slate-400"
        >
